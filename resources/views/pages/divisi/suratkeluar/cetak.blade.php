...
<body>
    ...
    <br>
    {{Fungsi::lembaga_kota()}}, {{Fungsi::tanggalindo($id->tgl)}}
    <br><br>
    Kepada Yth : Divisi Tujuan
    <br>
    Perihal : {{$id->perihal}}
...
<table width="100%">
    ...
    <tr>
        ...
        @php
            $pembuat='';
            $divisi_nama='';
            $getpembuat=\App\Models\User::with('divisi')->where('id',$id->users_id)->first();
            $pembuat=$getpembuat->name;
            $getdivisi=\App\Models\divisi::where('id',$id->divisi_id)->first();
            $divisi_nama=$getdivisi->nama!=null?$getdivisi->nama:'Divisi tidak ditemukan';
        @endphp
        <td width="30%" align="center">{{$pembuat}}</td>
    </tr>
    <tr>
        <td width="70%"></td>
        <td width="30%" align="center">{{$divisi_nama}}</td>
    </tr>
